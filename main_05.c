...
int main(void)
{
  ...
  nLis++;
	...
	printf("Testando o método de busca 3 Recursivo de um\n"); 
	printf(" elemento que esta na lista: 201913425\n");
	int achou = FALSE;
	strcpy(lista_din[nLis].numMatricula, "201913425");
	index = buscaLisAlunoOrdRec(lista_din, nLis, "201913425", &achou);
	if(achou){
	...
	achou = FALSE;
	strcpy(lista_din[nLis].numMatricula, "201900000");
	index = buscaLisAlunoOrdRec(lista_din, nLis, "201900000", &achou);
	if(achou){
    printf("Elemanto achado com índice %d\n", index);
		printItemLisAluno(lista_din, index);
  }else{
